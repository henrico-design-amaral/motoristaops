...
import { contextualWindows, contextSignalImpact, dynamicPricingImpact, contextCoverage, contextualModelMeta } from './context-signal-matrix';
...
const contextLabel=(date:string)=>contextByDate(date).map(x=>`${x.label} [${x.status}]`).join(' · ') || 'Sem contexto externo catalogado';

export const dailyContextReviews = [
  {date:'2026-08-13',day:dayCode('2026-08-13'),status:'reconstructed',trafficStatus:'CET específico não localizado; clima/aeroporto pendentes',operational:'R$ 274,42 na Uber; 13 corridas; 112,34 km em corrida; 4h52 em viagem.',regions:'Taboão, Butantã, Osasco, Congonhas, Barueri, Morumbi e eixo oeste/sul.',review:`Sequência corrida a corrida reconstruída. Intervalo estimado entre registros: ${Math.round(sequenceFor('2026-08-13')?.estimatedGapMin??0)} min. LAT.BUS entra como sinal de contexto no eixo Imigrantes; clima, CET específico e aeroporto permanecem desconhecidos quando não há evidência histórica confiável.`,sources:['Uber — histórico do dia','LAT.BUS 2026','Aena — referência operacional'],externalContext:contextLabel('2026-08-13')},
  ...
  {date:'2026-08-16',day:dayCode('2026-08-16'),status:'reconstructed',trafficStatus:'evento regional confirmado; clima/aeroporto pendentes',operational:'R$ 418,03 observados; 19 corridas + 1 cancelamento pago; 188,07 km em corrida; 5h42 em viagem.',regions:'Vila Sônia, Cotia, Centro, Ibirapuera, Santo Amaro, Saúde, Jardim Paulista e Lapa.',review:`Sequência corrida a corrida reconstruída. Intervalo estimado entre registros: ${Math.round(sequenceFor('2026-08-16')?.estimatedGapMin??0)} min. A Achiropita agora é casada por faixa e região; o sistema calcula deltas exploratórios contra janelas comparáveis, sem atribuição causal automática.`,sources:['Uber — histórico do dia','Prefeitura/CET — Festa da Achiropita','TSE — calendário eleitoral'],externalContext:contextLabel('2026-08-16')},
  {date:'2026-08-17',day:dayCode('2026-08-17'),status:'reconstructed',trafficStatus:'CET específico não localizado; clima/aeroporto pendentes',operational:'R$ 437,41 observados; 24 corridas + 1 cancelamento pago; 170,70 km em corrida; 8h18 em viagem.',regions:'Centro, Jardim Paulista, Santo Amaro, Vila Mariana, Jabaquara, Congonhas, Osasco e Carapicuíba.',review:`Sequência corrida a corrida reconstruída. Intervalo estimado entre registros: ${Math.round(sequenceFor('2026-08-17')?.estimatedGapMin??0)} min. Dinâmica já entra como sinal operacional observado; CET, chuva e aeroporto ficam explicitamente sem preenchimento quando a evidência histórica não está reconciliada.`,sources:['Uber — histórico do dia','Aena — referência operacional'],externalContext:contextLabel('2026-08-17')}
] as const;
...
export const predictiveIntelligence = {
  ...
  sample:{completeDays:historicalDays.length,detailedRides:completed.length,detailedRideDates:[...new Set(completed.map(r=>r.date))],recoveredDetailedRides:recoveredDetailed.length,recentEvidenceDays:latestEvidence.length,contextualWindows:contextualWindows.length},
  matrixMeta:rideMatrixMeta,
  warnings:['Ranking de dia da semana usa fechamentos completos históricos disponíveis.','Horário, rota e região combinam o dataset detalhado anterior com 13, 16 e 17/08 reconstruídos corrida por corrida.','estimatedGapMin e occupancyProxy são proxies derivados dos horários listados pela Uber; não são telemetria GPS.','goodWindowProbabilityProxy é score heurístico, não probabilidade calibrada.','Contexto externo só produz delta quando há evidência confirmada e casamento temporal/regional; pending/not-located nunca são convertidos em ausência real do fenômeno.'],
  nextModel:['Reconciliar chuva horária por estação/fonte histórica e anexar precipitação às janelas.','Ingerir ocorrências CET/acidente/obra por corredor com início/fim geográfico e temporal.','Reconstruir movimento/atrasos de Congonhas por faixa para medir demanda de aeroporto.','Adicionar km vazio e deslocamento até embarque para substituir occupancyProxy por ocupação operacional real.','Calibrar a probabilidade de janela boa com mais semanas e validação fora da amostra.']
} as const;
