...
4. **Status do protótipo.** Seis layouts editáveis existem no Figma, mas os três espaços de imagem ainda contêm placeholders: a conta Starter atingiu seu limite de chamadas antes de aplicar os assets. A tentativa de transportar o logo deixou dez camadas temporárias ocultas que deverão ser removidas na retomada da edição. A prova PDF de seis páginas, armazenada como `MotoristaOPS_Brand_Book_V2.5_Prova_Editorial.pdf`, já usa o logo institucional fornecido e duas fotos reais. Nem o Figma parcial nem o PDF representam livro completo, validação de impressão ou atualização do Brand System publicado.
